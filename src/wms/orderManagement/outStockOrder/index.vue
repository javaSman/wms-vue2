...
export default {
  ...
  data() {
    return {
      colName: 'OutStockAllDto',
      detailColName: 'OutStockDetail',
      dialogTitle: this.$t('dialogHeader.outStockOrder'), // '入库单',
      apiName: 'OutStock',
      ...
      detailTitle: this.$t('dialogHeader.stock'), // '物料',
  ...
  methods: {
    ...
    getDetail(row) {
      ...
      API.get('OutStock', { orderNo: row.orderNo }, 'GetOutStockDetailList').then(res => {
    ...
    handleDelete() {
      ...
      this.$confirm(this.$t('dialog.deleteTips'), this.$t('dialog.tip'), {
        ...
        .then(() => {
          ...
          API.dataPost('OutStock', ids, 'Delete')
            .then(res => {
              if (res.success) {
    ...
    getDict() {
      API.getDict('dict', { name: 'DeliveryOrderStatus' }).then(res => {
        this.outStockQueryItems[2].options = this.outStockFormList[1].options = res.details
      })
    },
    handleUpdate() {
      this.isEdit = true
      this.formTitle = '修改出库单管理'
