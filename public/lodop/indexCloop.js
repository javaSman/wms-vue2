;(function (win) {
  var CLODOPA = {
    ...
    OpenWebSocket: function () {
      if (!window.WebSocket && !window.MozWebSocket) {
        if (window.On_CLodop_Opened) {
          if (CLODOPA.Priority == window.CLODOPA_OK_Priority) setTimeout('window.On_CLodop_Opened(CLODOPA)', 1)
        }
        return
      }
      ...
      try {
        if (!this.webskt || this.webskt.readyState == 3) {
          ...
          this.webskt.onopen = function (e) {
            CLODOPA.SocketOpened = true
            if (window.On_CLodop_Opened) {
              if (CLODOPA.Priority == window.CLODOPA_OK_Priority) setTimeout('window.On_CLodop_Opened(CLODOPA)', 1)
            }
          }
          this.webskt.onmessage = function (e) {
            CLODOPA.blOneByone = false
            var strResult = e.data
            CLODOPA.Result = strResult
            try {
              var strFTaskID = null
              ...
              if (strFTaskID.indexOf('BroadcastMS') > -1) {
                CLODOPA.strBroadcastMS = strResult
                if (CLODOPA.On_Broadcast) {
                  var selfFunc = CLODOPA.On_Broadcast
                  CLODOPA.On_Broadcast(strResult)
                  if (!CLODOPA.On_Broadcast_Remain && selfFunc === CLODOPA.On_Broadcast) CLODOPA.On_Broadcast = null
                }
                return
              }
              if (CLODOPA.On_Return) {
                var selfFunc = CLODOPA.On_Return
                if (strResult.toLowerCase() == 'true' || strResult.toLowerCase() == 'false')
                  CLODOPA.On_Return(strFTaskID, strResult.toLowerCase() == 'true')
                else CLODOPA.On_Return(strFTaskID, strResult)
                if (!CLODOPA.On_Return_Remain && selfFunc === CLODOPA.On_Return) CLODOPA.On_Return = null
              }
            } catch (err) {}
          }
          this.webskt.onclose = function (e) {
            if (!CLODOPA.SocketOpened) {
              CLODOPA.SocketEnable = false
              return
            }
            setTimeout('CLODOPA.OpenWebSocket()', 2000)
          }
        ...
        else setTimeout('CLODOPA.OpenWebSocket()', 2000)
      }
    },
    ...
    PRINTAOK: function (iPrintIndex, iPrintCopies, iStartNO, iEndNO, onlySelect) {
      ...
      if (!onlySelect) this.PRINT()
        ...
        if (CLODOPA.On_Return) {
          var selfFunc = CLODOPA.On_Return
          CLODOPA.On_Return(0, iPrintIndex)
          if (!CLODOPA.On_Return_Remain && selfFunc === CLODOPA.On_Return) CLODOPA.On_Return = null
        }
      }
    },
    ...
    Create_PageSize_List: function (oElement, printIndex) {
      ...
      var strPageSizeList = CLODOPA.GET_PAGESIZES_LIST(printIndex, '\n')
    ...
    Get_AOPrinters_List: function (DriverNameOrIndex, strListName, strSplit) {
      ...
      return CLODOPA.GET_PAGESIZES_LIST(DriverNameOrIndex, strSplit, 'subdevlist', strListName)
    },
    Get_AOBridges_List: function (DriverNameOrIndex, strListName, strSplit) {
      return CLODOPA.Get_AOPrinters_List(DriverNameOrIndex, strListName, strSplit)
    },
    Get_AOBridge_List: function (strListName, strSplit) {
      return CLODOPA.Get_AOPrinters_List('(AO)Client-Bridge Driver', strListName, strSplit)
    },
      ...
      oscript.onload = oscript.onreadystatechange = function () {
        if (!oscript.readyState || /loaded|complete/.test(oscript.readyState)) {
          ...
          if (CLODOPA.On_Return)
            try {
              var selfFunc = CLODOPA.On_Return
              if (strResult == 'true' || strResult == 'false') CLODOPA.On_Return(strResultTaskID, strResult == 'true')
              else CLODOPA.On_Return(strResultTaskID, strResult)
              if (!CLODOPA.On_Return_Remain && selfFunc === CLODOPA.On_Return) CLODOPA.On_Return = null
            } catch (err) {}
    ...
    SelectBox: {
      ...
      selCopies: undefined,
      closeit: function () {
        if (CLODOPA.SelectBox.PopDiv && CLODOPA.SelectBox.PopDiv.parentNode)
          CLODOPA.SelectBox.PopDiv.parentNode.removeChild(CLODOPA.SelectBox.PopDiv)
      ...
      clickOK: function (onlySelect) {
        CLODOPA.PRINTAOK(CLODOPA.SelectBox.selPrinter.value, CLODOPA.SelectBox.selCopies.value, 0, 0, onlySelect)
        this.closeit()
      },
      create: function (iW, iH, onlySelect) {
        if (CLODOPA.SelectBox.PopDiv) this.closeit()
        var obody = document.body || document.getElementsByTagName('body')[0] || document.documentElement
        ...
        titleDiv.onmousedown = function (event) {
          CLODOPA.SelectBox.initializedrag(event)
        }
        titleDiv.onmousemove = function (event) {
          CLODOPA.SelectBox.drag_drop(event)
        }
        titleDiv.onmouseup = function () {
          CLODOPA.SelectBox.stopdrag()
        }
        ...
        CloseButton.onclick = function () {
          CLODOPA.SelectBox.closeit()
          if (onlySelect && CLODOPA.On_Return) {
            var selfFunc = CLODOPA.On_Return
            CLODOPA.On_Return(0, -1)
            if (!CLODOPA.On_Return_Remain && selfFunc === CLODOPA.On_Return) CLODOPA.On_Return = null
          }
        }
        ...
        var OKButton = CLODOPA.creatMyButtonElement('button', '确定')
        ...
        OKButton.onclick = function () {
          CLODOPA.SelectBox.clickOK(onlySelect)
        }
        var CancelButton = CLODOPA.creatMyButtonElement('button', '取消')
        ...
        areaDiv.appendChild(CLODOPA.creatLabelElement('span', '选打印机：', 200, 46, 67))
        ...
        CLODOPA.Create_Printer_List(oSelect)
        areaDiv.appendChild(CLODOPA.creatLabelElement('span', '打印份数：', 200, 46, 121))
        var oCopies = CLODOPA.creatMyButtonElement('text', '1')
        ...
        if (
          CLODOPA.Browser.IE &&
          (document.compatMode == 'BackCompat' || navigator.userAgent.indexOf('MSIE 6.0') > 0)
        ) {
    ...
    PreviewBox: {
      ...
      closeit: function (oSelf) {
        if (CLODOPA.PreviewBox.PopDiv && CLODOPA.PreviewBox.PopDiv.parentNode)
          CLODOPA.PreviewBox.PopDiv.parentNode.removeChild(CLODOPA.PreviewBox.PopDiv)
        if (this.FrantDiv && this.FrantDiv.parentNode) this.FrantDiv.parentNode.removeChild(this.FrantDiv)
        this.PopDiv = undefined
      },
      ...
      create: function (strURL, iW, iH) {
        if (CLODOPA.PreviewBox.PopDiv) this.closeit()
        var obody = document.body || document.getElementsByTagName('body')[0] || document.documentElement
        ...
        vBoxDiv.onmousedown = function (event) {
          CLODOPA.PreviewBox.initializedrag(event, this)
        }
        vBoxDiv.onmouseup = function () {
          CLODOPA.PreviewBox.stopdrag()
        }
        vBoxDiv.onmousemove = function (event) {
          CLODOPA.PreviewBox.drag_drop(event, this)
        }
        this.PopDiv = vBoxDiv
        var titleDiv = document.createElement('div')
        ...
        CloseButton.style.cssText =
          ...
          '/images/c_winclose.png) no-repeat scroll 0 0px;margin-right:5px;position:absolute;height:20px;line-height:100px;width:34px;right:3px;border:0;top:4px'
        CloseButton.onclick = function () {
          CLODOPA.PreviewBox.closeit(this)
        }
        var areaDiv = document.createElement('div')
  ...
  if (win.CLODOPA2015_7028 && win.CLODOPA2015_7028.Priority && win.CLODOPA2015_7028.Priority > CLODOPA.Priority) {
    CLODOPA = win.CLODOPA2015_7028
    win.CLODOPA_OK_Priority = win.CLODOPA2015_7028.Priority
    return
  }
  win.LODOP = CLODOPA
  win.CLODOPA = CLODOPA
  win.CLODOPA2015_7028 = CLODOPA
  win.CLODOPA_OK_Priority = CLODOPA.Priority
  win.CLODOPA.DoInit()
  if (navigator.userAgent.indexOf('Lodop') < 0) win.CLODOPA.OpenWebSocket()
})(window)

function getCLodop() {
  return window.CLODOPA2015_7028
}
