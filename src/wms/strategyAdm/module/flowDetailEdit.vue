...
export default {
  ...
  methods: {
    handleFilter() {
      ...
      this.dataList = this.detailAll.filter(Functions.vagueQuery({ gatherNo: this.gatherNo }))
    ...
    save() {
      let gatherIDs = [...new Set(this.multipleSelection.map(row => row.gatherNo))]
      let same = this.detailAll.filter(row => {
        let hasGatherID = gatherIDs.find((val, i, arr) => {
          return val === row.gatherNo
        })
