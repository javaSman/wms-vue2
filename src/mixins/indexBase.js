...
import { baseAPI } from '@/api/generalAPI'
...
export default {
  ...
  methods: {
    ...
    getList(val) {
      ...
      baseAPI.getData(this.apiName, this.listQuery, null).then(res => {
    ...
    getDetail(row) {
      ...
      this.isEdit = true
      baseAPI.getForm(this.apiName, row.id).then(res => {
    ...
    /** 删除，默认以id字段提交 */
    handleDelete() {
      this.delete('id', baseAPI.dataPost) // 可在页面自定义
    },
    delete(id, api) {
      var params = []
    ...
    exportPast(name, params, otherExport, result) {
      // debugger
      this.downloadLoading = true
      let other = otherExport || 'Export'
      API.dataPost(name, params, other)
        .then(res => {
          if (res !== '') {
            let suffix = result ? res.result : res
            var url = window.globalConfig.base.ip + ':' + window.globalConfig.base.backend_port + suffix
            window.location.href = url
            this.$notify({
              title: this.$t('notify.success'), // '成功'
              message: this.$t('notify.exportSuccess'), // '导出成功'
